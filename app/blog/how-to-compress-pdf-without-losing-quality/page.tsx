...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/blog/how-to-compress-pdf-without-losing-quality' },
  openGraph: {
    ...
    <div className="bg-black min-h-screen text-white flex flex-col justify-between selection:bg-purple-500/30">
      ...
      <article className="max-w-3xl mx-auto px-6 py-32 space-y-10 flex-1 w-full">
        ...
        <div className="text-white/70 space-y-6 leading-relaxed text-sm sm:text-base">
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Why PDF Files Get Large</h2>
          <p>
          ...
          <ul className="list-disc list-inside space-y-2 pl-4 text-sm">
            <li><strong className="text-white">Scanned PDFs</strong> are essentially image files in a PDF wrapper — each page is a large raster image at 300–600dpi.</li>
            <li><strong className="text-white">Word processor exports</strong> embed full font files, metadata, and revision history.</li>
            <li><strong className="text-white">Design exports</strong> include colour profiles, embedded thumbnails, and uncompressed vector paths.</li>
          </ul>

          <h2 className="text-xl font-bold text-white pt-4 font-display">4 Expert Tips for Better Compression</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-4">
            {tips.map((t, i) => (
              <div key={i} className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-2">
                <span className="text-xs font-bold text-purple-400">Tip {i + 1}</span>
                <h3 className="font-bold text-white text-sm">{t.title}</h3>
                <p className="text-xs text-white/50 leading-normal">{t.desc}</p>
              </div>
            ))}
          </div>

          <h2 className="text-xl font-bold text-white pt-4 font-display">How to Compress a PDF Using iCreatePDF</h2>
          <ol className="list-decimal list-inside space-y-3 pl-4 text-sm">
            <li>Go to <Link href="/compress-pdf" className="text-purple-400 hover:underline">/compress-pdf</Link> — no sign-up needed.</li>
            <li>Drop your PDF onto the upload zone.</li>
            <li>Choose a compression level: <strong className="text-white">High</strong> (best for text-heavy docs), <strong className="text-white">Medium</strong> (balanced), or <strong className="text-white">Low</strong> (preserves print-quality images).</li>
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Common Compression Myths — Busted</h2>
          <div className="space-y-4">
            {myths.map((m, i) => (
              <div key={i} className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-1">
                <p className="text-xs text-red-400 line-through">{m.myth}</p>
                <div className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                  <p className="text-xs text-white/70 leading-relaxed">{m.truth}</p>
                </div>
              </div>
            ))}
          </div>

          <h2 className="text-xl font-bold text-white pt-4 font-display">What File Size Should You Target?</h2>
              ...
              <tbody className="divide-y divide-white/5">
                {[
                  ...
                  <tr key={use}>
                    ...
                    <td className="px-4 py-3 text-purple-400 font-mono">{size}</td>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-purple-950/20 to-pink-950/20 border border-purple-500/20 text-center space-y-4">
          <h3 className="text-lg font-bold text-white font-display">Ready to shrink your PDF?</h3>
          <p className="text-xs text-white/60 max-w-md mx-auto">100% client-side compression — your files never leave your browser.</p>
