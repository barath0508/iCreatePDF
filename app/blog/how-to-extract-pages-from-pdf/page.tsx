...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/blog/how-to-extract-pages-from-pdf' },
  openGraph: {
    ...
    <div className="bg-black min-h-screen text-white flex flex-col justify-between selection:bg-purple-500/30">
      ...
      <article className="max-w-3xl mx-auto px-6 py-32 space-y-10 flex-1 w-full">
        ...
        <div className="text-white/70 space-y-6 leading-relaxed text-sm sm:text-base">
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Three Ways to Clean Up PDF Pages</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-4">
            {methods.map((m, i) => (
              <div key={i} className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-2">
                ...
                <p className="text-xs text-white/50 leading-normal">{m.desc}</p>
              </div>
            ))}
          </div>

          <h2 className="text-xl font-bold text-white pt-4 font-display">How to Extract Pages Instantly</h2>
          <p>
            Using iCreatePDF\'s page extractor, you can input a page range (e.g., <code className="text-purple-400 font-mono">1, 3-5, 10</code>) and generate a new PDF containing only those selected pages.
          </p>
          <ol className="list-decimal list-inside space-y-2 pl-4 text-sm">
            <li>Go to <Link href="/extract-pages" className="text-purple-400 hover:underline">/extract-pages</Link></li>
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Trimming &amp; Formatting: Crop and Resize</h2>
          <p>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-1">
              <h4 className="text-sm font-semibold text-white">Does extracting pages reduce quality?</h4>
              <p className="text-xs text-white/50 leading-relaxed">No. The text layers, vector images, and font styles are copied directly from the source pages, meaning the output has the exact same resolution and format.</p>
            </div>
            <div className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-1">
              <h4 className="text-sm font-semibold text-white">Can I extract pages from a protected PDF?</h4>
              <p className="text-xs text-white/50 leading-relaxed">If a PDF is locked with a password, you must first unlock it using the Unlock PDF tool, then proceed to extract pages.</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-purple-950/20 to-pink-950/20 border border-purple-500/20 text-center space-y-4">
          ...
          <p className="text-xs text-white/60 max-w-md mx-auto">100% browser-based. Zero uploads. Complete page control.</p>
