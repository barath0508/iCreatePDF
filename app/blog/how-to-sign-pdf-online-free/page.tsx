...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/blog/how-to-sign-pdf-online-free' },
  openGraph: {
    ...
    <div className="bg-black min-h-screen text-white flex flex-col justify-between selection:bg-purple-500/30">
      ...
      <article className="max-w-3xl mx-auto px-6 py-32 space-y-10 flex-1 w-full">
        ...
        <div className="text-white/70 space-y-6 leading-relaxed text-sm sm:text-base">
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">3 Types of PDF Signatures — Which Do You Need?</h2>
          <div className="space-y-4 my-4">
            {signatureTypes.map((s) => (
              <div key={s.type} className="p-4 rounded-xl bg-zinc-950 border border-white/5 flex gap-4">
                <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-lg shrink-0 font-bold text-purple-400">
                  {s.icon}
                </div>
                <div className="space-y-1">
                  <h3 className="font-bold text-white text-sm">{s.type}</h3>
                  <p className="text-xs text-white/50 leading-normal">{s.desc}</p>
                  <p className="text-[10px] text-purple-400 font-mono uppercase tracking-wide">Best for: {s.best}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Step-by-Step: Sign a PDF With iCreatePDF</h2>
          <ol className="list-decimal list-inside space-y-3 pl-4 text-sm">
            <li>Go to <Link href="/sign-pdf" className="text-purple-400 hover:underline">iCreatePDF Sign PDF</Link>.</li>
            <li>Upload the PDF you need to sign.</li>
            <li>Click <strong className="text-white">Add Signature</strong> — choose to draw, type, or upload an image of your signature.</li>
            ...
            <li>Click <strong className="text-white">Apply &amp; Download</strong> — the signature is burned into the PDF and ready to share.</li>
          </ol>
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Is a Browser-Based Signed PDF Legally Valid?</h2>
          <p>
          ...
          <p>
            The visual signature approach (draw/type) qualifies as a <strong className="text-white">Simple Electronic Signature (SES)</strong> under eIDAS and is valid for most everyday contracts, agreements, and acknowledgements. For high-value legal transactions requiring a Qualified Electronic Signature (QES), you would need a certificate from a Trust Service Provider (TSP).
          </p>

          <h2 className="text-xl font-bold text-white pt-4 font-display">How to Verify a Signed PDF</h2>
          <p>
            Once a document has been signed with a cryptographic certificate, you can verify its authenticity using iCreatePDF's <Link href="/verify-signature" className="text-purple-400 hover:underline">Verify Signature</Link> tool. It reads the embedded PKCS#7 signature block, extracts the signer identity and timestamp, and performs a byte-range integrity check — all locally in your browser.
          </p>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-purple-950/20 to-pink-950/20 border border-purple-500/20 text-center space-y-4">
          <h3 className="text-lg font-bold text-white font-display">Sign your PDF now — free &amp; private</h3>
          <p className="text-xs text-white/60 max-w-md mx-auto">Draw, type, or upload your signature. Files never leave your browser.</p>
          <Link href="/sign-pdf">
