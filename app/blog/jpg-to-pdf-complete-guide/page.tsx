...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/blog/jpg-to-pdf-complete-guide' },
  openGraph: {
    ...
    <div className="bg-black min-h-screen text-white flex flex-col justify-between selection:bg-purple-500/30">
      ...
      <article className="max-w-3xl mx-auto px-6 py-32 space-y-10 flex-1 w-full">
        ...
        <div className="text-white/70 space-y-6 leading-relaxed text-sm sm:text-base">
          ...
          <h2 className="text-xl font-bold text-white pt-4 font-display">Supported Image Formats</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-4">
            {formats.map((f) => (
              <div key={f.ext} className="p-3 rounded-xl bg-zinc-950 border border-white/5 flex items-start gap-3">
                <span className="text-xs font-bold text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded font-mono shrink-0">{f.ext}</span>
                <p className="text-xs text-white/50 leading-normal">{f.desc}</p>
              </div>
            ))}
          </div>

          <h2 className="text-xl font-bold text-white pt-4 font-display">Step-by-Step: Convert JPG to PDF</h2>
          <ol className="list-decimal list-inside space-y-3 pl-4 text-sm">
            <li>Open <Link href="/jpg-to-pdf" className="text-purple-400 hover:underline">iCreatePDF — JPG to PDF</Link>.</li>
            ...
            <li>Configure your <strong className="text-white">PDF settings</strong> in the panel on the right (page size, orientation, quality, margins).</li>
            <li>Click <strong className="text-white">Convert to PDF</strong> and download your file. Done.</li>
          </ol>

          <h2 className="text-xl font-bold text-white pt-4 font-display">PDF Settings Explained</h2>
          <div className="space-y-3 my-4">
            {settings.map((s) => (
              <div key={s.name} className="p-4 rounded-xl bg-zinc-950 border border-white/5 grid grid-cols-3 gap-4">
                <div>
                  <p className="text-xs font-bold text-white">{s.name}</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 font-mono leading-relaxed">{s.options}</p>
                </div>
                <div>
                  <p className="text-[10px] text-purple-400/80 leading-relaxed">{s.tip}</p>
                </div>
              </div>
            ))}
          </div>

          <h2 className="text-xl font-bold text-white pt-4 font-display">Why Convert Images to PDF?</h2>
          <p>
          ...
          <ul className="list-disc list-inside space-y-2 pl-4 text-sm">
            <li><strong className="text-white">Fixed layout</strong> — PDFs look identical on every device and screen size.</li>
            <li><strong className="text-white">Multi-page support</strong> — bundle dozens of images into a single file.</li>
            <li><strong className="text-white">Universal compatibility</strong> — every operating system, phone, and web browser can open a PDF.</li>
            <li><strong className="text-white">Compression</strong> — a PDF can be significantly smaller than the sum of its source images.</li>
            <li><strong className="text-white">Professional presentation</strong> — PDFs are the accepted standard for job applications, legal documents, academic submissions, and business proposals.</li>
          </ul>

          <h2 className="text-xl font-bold text-white pt-4 font-display">Tips for the Best Output Quality</h2>
          <div className="space-y-4">
            {[
              ...
              <div key={t.tip} className="p-4 rounded-xl bg-zinc-950 border border-white/5 space-y-1">
                <p className="text-sm font-bold text-white">💡 {t.tip}</p>
                <p className="text-xs text-white/50 leading-relaxed pl-5">{t.detail}</p>
              </div>
            ))}
          </div>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-r from-orange-950/20 to-purple-950/20 border border-orange-500/20 text-center space-y-4">
          <h3 className="text-lg font-bold text-white font-display">Convert your images to PDF now</h3>
          <p className="text-xs text-white/60 max-w-md mx-auto">JPG, PNG, HEIC, WEBP — any format, any device, always free.</p>
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <Link href="/jpg-to-pdf">
              <Button className="bg-orange-600 hover:bg-orange-700 text-white font-medium text-xs px-5 rounded-full group">
                JPG to PDF
                <ArrowRight className="w-3.5 h-3.5 ml-1 transition-transform group-hover:translate-x-1" />
              </Button>
            </Link>
            <Link href="/png-to-pdf">
              <Button variant="outline" className="border-white/20 text-white hover:bg-white/5 font-medium text-xs px-5 rounded-full">
                PNG to PDF
              </Button>
            </Link>
