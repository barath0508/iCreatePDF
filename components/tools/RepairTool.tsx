      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-6">
          {!file ? (
            ...
            >
              ...
              <div className="p-4 rounded-full bg-white/5 mb-4 border border-white/10"><Wrench className="w-6 h-6 text-purple-400" /></div>
              <h3 className="text-xl font-display text-white mb-2">Upload a corrupted PDF</h3>
              <p className="text-xs text-white/40">The tool will rebuild cross-reference tables and recover readable content.</p>
            </div>
          ) : (
            <div className="p-6 bg-zinc-950 border border-white/10 rounded-2xl space-y-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-xl"><Wrench className="w-5 h-5 text-purple-400" /></div>
                  <div>
                    <h4 className="text-base font-display text-white">{file.name}</h4>
                    <p className="text-xs text-white/40">{fmt(file.size)}</p>
                  </div>
                </div>
                <Button variant="ghost" size="sm" onClick={() => { setFile(null); setDownloadUrl(null); setResult(null); }} className="text-xs text-white/40 hover:text-white">Change</Button>
              </div>
              {result && (
        ...
        <div className="lg:col-span-4 bg-zinc-950 border border-white/10 rounded-2xl p-6 space-y-6">
          <div className="flex items-center gap-2 border-b border-white/5 pb-4">
            ...
            <h3 className="font-mono text-sm uppercase tracking-wider text-white">Repair PDF</h3>
          </div>
          <p className="text-xs text-white/50 leading-relaxed">Attempts to recover PDFs with broken cross-reference tables, truncated streams, or invalid object entries by rebuilding the document structure.</p>
          ...
          <div className="pt-4 border-t border-white/5">
            {isProcessing ? (
              ...
              <div className="space-y-2">
                ...
                <Button variant="ghost" onClick={() => { setFile(null); setDownloadUrl(null); setResult(null); }} className="w-full text-white/50 hover:text-white text-xs h-8">Repair another file</Button>
              </div>
            ) : (
