    ...
    <div className="w-full max-w-6xl mx-auto px-6 lg:px-12 py-16">
      ...
      {mode === 'certify' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          <div className="lg:col-span-8 space-y-6 flex flex-col">
            {!verifyFile ? (
              <div onClick={() => verifyInputRef.current?.click()} className="cursor-pointer touch-manipulation border border-dashed rounded-2xl p-12 transition-all text-center flex-1 flex flex-col items-center justify-center min-h-[220px] border-foreground/10 bg-card/40 hover:border-foreground/20">
                <input type="file" ref={verifyInputRef} onChange={(e) => e.target.files && handleVerifyFiles(e.target.files)} accept="application/pdf,.pdf" onClick={(e) => {
                e.stopPropagation();
                (e.target as HTMLInputElement).value = '';
              }}
              className="hidden" />
